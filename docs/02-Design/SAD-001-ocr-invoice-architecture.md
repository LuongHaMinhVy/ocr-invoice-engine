...
graph TD
    ...
    subgraph "FastAPI Backend (apps/backend)"
        direction TB
...
├── apps/
│   ├── backend/             # Dự án FastAPI (Python 3.11/3.12)
│   │   ├── requirements.txt
│   │   ├── config.py        # Cấu hình Mail, Database, Gemini API
│   │   ├── main.py          # Entrypoint FastAPI (Uvicorn)
│   │   ├── app/
│   │   │   ├── __init__.py  # Khởi tạo package
│   │   │   ├── database.py  # Cấu hình DB engine & sessionmaker
│   │   │   ├── routers/     # API Route Routers (invoices.py)
...
### 5.1 Exception Handling ở Backend (FastAPI)
- **`exception_handler`:** Sử dụng `@app.exception_handler` toàn cục để bắt các lỗi như `HTTPException`, `RequestValidationError` và custom exceptions. Trả về mã lỗi HTTP chuẩn hóa kèm thông báo dạng JSON:
  ```json
  {
...
- **Giải pháp:** Backend FastAPI bắt buộc phải cung cấp endpoint `/api/invoices/{invoice_id}/image` để stream file ảnh dưới dạng nhị phân thông qua `FileResponse` từ thư mục tải lên cấu hình trước.
...
- **Ràng buộc:** Để tránh việc auto-reloader của uvicorn/gunicorn khởi động Scheduler nhiều lần tại local hoặc chạy nhiều luồng polling song song khi chạy multi-worker ở Production:
- **Giải pháp:** 
  - Tại local dev: Khi khởi chạy với Uvicorn, không bật tham số `--reload` nếu chạy kèm scheduler, hoặc chạy scheduler bằng cách lắng nghe sự kiện `startup` của FastAPI một lần duy nhất.
  - Quy mô hệ thống: Thiết kế tích hợp này chỉ chạy ở mức độ vừa và nhỏ. Khi scale lớn, email polling phải được chạy độc lập như một daemon riêng.

### 8.4 Ràng buộc Migration & Bảng dữ liệu
- **Ràng buộc:** Triển khai đơn giản và nhanh chóng trên Dev, việc tạo bảng sẽ dùng `Base.metadata.create_all(bind=engine)` bên trong khối lệnh khởi chạy ứng dụng (hoặc lúc startup).
